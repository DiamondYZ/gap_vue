  ...
  <div class="app-container">
    <el-card class="operate-container" shadow="never">
            ...
            <div style="margin-top: 15px">
              <el-form :inline="true" :model="search_data" size="small" label-width="140px">
                ...
                <el-form-item label="机具类型：">
                  <el-input v-model="search_data.implementTypeId" style="width: 203px" placeholder="" />
                </el-form-item>
                <el-form-item label="机具型号：">
                  <el-input v-model="search_data.implementModelId" style="width: 203px" placeholder="" />
                </el-form-item>
              </el-form>
            </div>
          </el-card>
      ...
      <show-detail-form
        ref="detailForm"
        :page-info="pageInfo"
        :form-configs="implementFormConfigs"
...
import { implementFormConfigs } from '../../components/Form/form-configs.js'
...
export default {
  ...
  data() {
    return {
      pageInfo: {
        interfaceName: 'implement', // 接口名称
        listTitle: '机具列表',
        detailTitle: '机具详细信息'
      }, // 页面信息
      dialogTypeInfo: {
        selectDialogTitle: '机具类型',
        dialogAxiosName: 'implementType',
        dialogId: 'implementTypeId',
        selectOptions: [],
        tableTitleList: []
      },
      dialogModelInfo: {
        selectDialogTitle: '机具型号',
        dialogAxiosName: 'implementModel',
        dialogId: 'implementModelId',
        selectOptions: [],
        tableTitleList: []
      },
  ...
  watch: {
    listeningClickDialog(val) {
      if (val) {
        let param
        let urlValue
        if (val === 'implementType') {
          urlValue = 'implement-type'
        } else if (val === 'implementModel') {
          urlValue = 'implement-model'
        ...
        this.$store.dispatch('common/getSelectOptionsList', param).then((res) => {
          if (val === 'implementType') {
            ...
            this.$refs.dialogType.showTable()
          }
          if (val === 'implementModel') {
            ...
            this.$refs.dialogModel.showTable()
          }
          if (val === 'productionBase') {
            ...
            this.$refs.dialogProductionBase.showTable()
          }
        })
          .catch(() => {
