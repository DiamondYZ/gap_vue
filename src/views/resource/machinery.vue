  ...
  <div class="app-container">
    <el-card class="operate-container" shadow="never">
            ...
            <div style="margin-top: 15px">
              <el-form :inline="true" :model="search_data" size="small" label-width="140px">
                ...
                <el-form-item label="物料类型：">
                  <el-input v-model="search_data.materialTypeId" style="width: 203px" placeholder="" />
                </el-form-item>
              </el-form>
            </div>
          </el-card>
      ...
      <show-detail-form
        ref="detailForm"
        :page-info="pageInfo"
        :form-configs="materialFormConfigs"
        @getList="getList(nowPage)"
      />
      <dialog-table
        ref="dialogMaterialType"
        :dialog-info="dialogMaterialTypeInfo"
        :form-configs="materialFormConfigs"
...
import { materialFormConfigs } from '../../components/Form/form-configs.js'
...
export default {
  ...
  data() {
    return {
      pageInfo: {
        interfaceName: 'material', // 接口名称
        listTitle: '物料列表',
        detailTitle: '物料详细信息'
      }, // 页面信息
      dialogMaterialTypeInfo: {
        selectDialogTitle: '物料类型',
        dialogAxiosName: 'materialType',
        dialogId: 'materialTypeId',
        selectOptions: [],
        tableTitleList: []
      },
      dialogSupplierInfo: {
        selectDialogTitle: '供应商',
        dialogAxiosName: 'supplier',
        dialogId: 'supplierId',
        selectOptions: [],
        tableTitleList: []
      },
      ...
      tableTitleList: [
        ...
        { prop: 'materialTypeName', name: '物料类型' },
        { prop: 'sourceDict', name: '来源' },
        { prop: 'isDesignateSupplier', name: '是否指定供应商' }
      ], // 表格头信息
      tableData: [], // 表格数据
      formData: {}, // 表单数据
      loading: false,
  ...
  watch: {
    listeningClickDialog(val) {
      if (val) {
        ...
        const param = {
          url: url + '/getPullDownList'
        }
        this.$store.dispatch('common/getSelectOptionsList', param).then((res) => {
          if (val === 'materialType') {
            this.dialogMaterialTypeInfo.tableTitleList = [
              { prop: 'number', name: '类型编号' },
              { prop: 'name', name: '类型名称' },
              { prop: 'description', name: '类型描述' }
            ] // 表格头信息
            this.dialogMaterialTypeInfo.selectOptions = res.data
            this.$refs.dialogMaterialType.showTable()
          }
          if (val === 'supplier') {
            ...
            this.$refs.dialogSupplier.showTable()
          }
        })
          .catch(() => {
  ...
  created() {
    this.$store.dispatch('common/getPullDownList', { classCode: 'MATERIAL_SOURCE_DICT' }) // 来源
    this.$store.dispatch('common/getPullDownList', { classCode: 'IS_VALID' }) // 是否指定供应商
    // let param = {
    //     url: 'material-type/getList',
    //     storageName: 'materialTypeSelect'
    // }
    // this.$store.dispatch('common/getSelectList', param)   // 物料类型
    // let param2 = {
    //     url: 'supplier/getList',
    //     storageName: 'supplierSelect'
    // }
    // this.$store.dispatch('common/getSelectList', param2)   // 供应商
  },
  mounted() {
    this.getList()
  },
  methods: {
    formatRole(row, column) {
      if (column.property === 'sourceDict') {
        const statusArr = JSON.parse(localStorage.getItem('MATERIAL_SOURCE_DICT'))
        return this.getArrayMapVal(statusArr, row[column.property])
      } else if (column.property === 'isDesignateSupplier') {
