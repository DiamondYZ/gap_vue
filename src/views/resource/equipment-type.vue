  ...
  <div class="app-container">
    <el-card class="operate-container" shadow="never">
            ...
            <div style="margin-top: 15px">
              <el-form :inline="true" :model="search_data" size="small" label-width="140px">
                ...
                <el-form-item label="是否可用：">
                  <el-select class="item-choose" v-model="search_data.isValid" size="small">
      ...
      <show-detail-form ref="detailForm" :pageInfo=pageInfo
                        :formConfigs="equipmentTypeFormConfigs"
...
<script>
  ...
  import { equipmentTypeFormConfigs, formData } from '../../components/Form/form-configs.js'

  export default {
    data() {
      return {
        pageInfo: {
          interfaceName: 'equipment-type',   //接口名称
          listTitle: '设备类型列表',
          detailTitle: '设备类型详细信息'
        },   //页面信息
        search_data: {},          //搜索条件
        clickLineId: '',       //当前点击行id
    ...
    computed: {},
    created(){
      this.$store.dispatch('common/getPullDownList', { classCode: 'ABC_CLASS' })   // ABC分类
      this.$store.dispatch('common/getPullDownList', { classCode: 'EQUIPMENT_USE_CLASS' })   // 设备使用类型
      this.$store.dispatch('common/getPullDownList', { classCode: 'EQUIPMENT_M_TYPE' })   // 设备管理类型
    },
    mounted() {
      this.getList()
    },
