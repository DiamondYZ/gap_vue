...
<!--                  <el-input v-model="search_data.productTypeId" style="width: 203px" placeholder="" />-->
<!--                </el-form-item>-->
              </el-form>
            </div>
          </el-card>
...
export default {
  ...
  data() {
    return {
      ...
      tableTitleList: [
        ...
        { prop: 'storageUnitDict', name: '存储单位' },
        { prop: 'isSpecial', name: '是否特殊需求' }
      ], // 表格头信息
      tableData: [], // 表格数据
      formData: {}, // 表单数据
      loading: false,
        ...
        this.$store.dispatch('common/getSelectOptionsList', param).then((res) => {
          ...
          if (val === 'productType') {
            this.dialogInfo.tableTitleList = [
              { prop: 'number', name: '产品类型编号' },
              { prop: 'name', name: '产品类型名称' },
              { prop: 'description', name: '产品描述' }
            ] // 表格头信息
          }
  ...
  methods: {
    formatRole(row, column) {
      ...
      } else if (column.property === 'storageUnitDict') {
        const statusArr = JSON.parse(localStorage.getItem('STORAGE_UNIT_DICT'))
        return this.getArrayMapVal(statusArr, row[column.property])
      } else {
