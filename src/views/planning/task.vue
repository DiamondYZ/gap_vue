    ...
    <el-card class="operate-container" shadow="never">
      ...
      <div style="float: right;margin:20px 30px">
        <el-button type="success" size="small" @click="setStatus('issued')"
                   v-bind:disabled="statusDict!=='saved'">下发
        </el-button>
...
<script>
    ...
    export default {
            ...
            listeningClickDialog(val) {
                if (val) {
                    ...
                    this.$store.dispatch('common/getSelectOptionsList', param).then((res) => {
                        ...
                        if (val === 'taskType') {
                            this.dialogTaskTypeInfo.selectOptions = res.data
                            this.dialogTaskTypeInfo.tableTitleList = [
                                {prop: 'taskTypeName', name: '任务类型'},
                                {prop: 'number', name: '编号'},
                                {prop: 'name', name: '名称'},
                                {prop: 'description', name: '描述'}
                            ] // 表格头信息
                            this.$refs.dialogTaskTypeSelectTable.showTable()
                        }
                    })
                        .catch(() => {
        ...
        created() {
            this.$store.dispatch('common/getPullDownList', {classCode: 'DISPATCH_LIST_STATUS'}) // 任务状态
