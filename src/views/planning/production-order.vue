  ...
  <div class="app-container">
    <el-card class="operate-container" shadow="never">
      ...
      <div style="float: right;margin:20px 30px">
        <el-button type="primary" size="small" @click="setStatus('confirmed')"
                   v-bind:disabled="statusDict!=='planned'">确认
        </el-button>
        <el-button type="success" size="small" @click="setStatus('issued')"
                   v-bind:disabled="statusDict!=='confirmed'">下达
        </el-button>
        <el-button type="info" size="small" plain @click="setStatus('completed')"
                   v-bind:disabled="statusDict!=='issued'">完成
        </el-button>
      ...
      <show-detail-form
        ref="detailForm"
        :page-info="pageInfo"
        :form-configs="productionOrderFormConfigs"
...
<script>
    ...
    import {productionOrderFormConfigs, formData} from '../../components/Form/form-configs.js'
    ...
    export default {
        ...
        data() {
            return {
                pageInfo: {
                    interfaceName: 'production-order', // 接口名称
                    listTitle: '生产订单列表',
                    detailTitle: '生产订单详细信息'
                }, // 页面信息
                dialogProductInfo: {
        ...
        created() {
            this.$store.dispatch('common/getPullDownList', {classCode: 'PRODUCTION_ORDER_STATUS'}) // 生产订单状态
            this.$store.dispatch('common/getPullDownList', {classCode: 'QUANTITY_UNIT_DICT'}) // 数量单位
        },
        methods: {
            formatRole(row, column) {
                if (column.property === 'statusDict') {
                    const statusArr = JSON.parse(localStorage.getItem('PRODUCTION_ORDER_STATUS'))
