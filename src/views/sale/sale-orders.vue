  ...
  <div class="app-container">
    <el-card class="operate-container" shadow="never">
      ...
      <div style="float: right;margin:20px 30px">
        <el-button type="primary" size="small" :disabled="statusDict!=='unissued'" @click="setStatus('issued')">发布
        </el-button>
        <el-button type="success" size="small" :disabled="statusDict!=='issue'" @click="setStatus('deliver')">交付
        </el-button>
        <el-button type="info" size="small" :disabled="statusDict!=='deliver'" @click="setStatus('settled')">结算
        </el-button>
        <el-button
          type="warning"
          size="small"
          :disabled="statusDict==='cancel'||statusDict==='settled'||statusDict===''"
      ...
      <show-detail-form
        ref="detailForm"
        :page-info="pageInfo"
        :form-configs="saleOrderFormConfigs"
      ...
      />
      <dialog-table
        ref="dialogConsumerSelectTable"
        :dialog-info="dialogConsumerInfo"
        :form-configs="saleOrderFormConfigs"
        :table-title="tableTitle"
      />
      <dialog-table
...
import { saleOrderFormConfigs, formData } from '../../components/Form/form-configs.js'
...
export default {
  ...
  data() {
    return {
      pageInfo: {
        interfaceName: 'sale-order', // 接口名称
        listTitle: '销售订单列表',
        detailTitle: '销售订单详细信息',
        listDetailTitle: '销售订单明细列表'
      }, // 页面信息
      dialogSalesmanInfo: {
        selectDialogTitle: '负责人',
        dialogAxiosName: 'salesman',
        dialogId: 'salesmanId',
        selectOptions: [],
        tableTitleList: []
      },
      dialogConsumerInfo: {
        selectDialogTitle: '客户',
        dialogAxiosName: 'consumer',
        dialogId: 'consumerId',
        selectOptions: [],
        tableTitleList: []
      },
      dialogSupplierInfo: {
        selectDialogTitle: '供货方',
        dialogAxiosName: 'company',
        dialogId: 'companyId',
        selectOptions: [],
        tableTitleList: []
      },
      ...
      tableTitleList: [
        ...
        { prop: 'consumerName', name: '客户名称' },
        { prop: 'salesmanName', name: '销售负责人名称' },
        { prop: 'sumMoney', name: '总金额' },
    ...
    listeningClickDialog(val) {
      if (val) {
        ...
        this.$store.dispatch('common/getSelectOptionsList', param).then((res) => {
          ...
          if (val === 'consumer') {
            this.dialogConsumerInfo.selectOptions = res.data
            this.dialogConsumerInfo.tableTitleList = [
              { prop: 'number', name: '编号' },
              { prop: 'name', name: '名称' },
              { prop: 'description', name: '描述' },
              { prop: 'managerName', name: '负责人' }
            ] // 表格头信息
            this.$refs.dialogConsumerSelectTable.showTable()
          }
          if (val === 'company') {
  ...
  methods: {
    formatRole(row, column) {
      if (column.property === 'statusDict') {
        const statusArr = JSON.parse(localStorage.getItem('SALE_ORDER_STATUS'))
